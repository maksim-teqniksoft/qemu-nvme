 ...
 * (qemu) device_add nvme,drive=<new_drive_id>,serial=<serial>,id=<new_id>[,...]
 *
...
{
    if (nsid == 0 || nsid > le32_to_cpu(n->id_ctrl.nn)) {
        return NVME_INVALID_NSID | NVME_DNR;
    } else if (n->ns_all[nsid - 1] == NULL) {
        return NVME_INVALID_NSID | NVME_DNR;
    } else if (n->ns_attached[nsid - 1] == NULL) {
                             ...
                             uint32_t len, NvmeCtrl *n)
{
    ...
    if (!prp1) {
        ...
        qemu_iovec_add(iov, &n->cmbuf[prp1 - n->ctrl_mem.addr], trans_len);
    } else {
        ...
        if (len > n->page_size) {
            ...
            while (len != 0) {
                ...
                if (!cmb) {
                    ...
                    qemu_iovec_add(iov, &n->cmbuf[prp_ent - n->ctrl_mem.addr],
                                   trans_len);
                }
                len -= trans_len;
                i++;
            ...
            if (!cmb) {
                ...
                qemu_iovec_add(iov, &n->cmbuf[prp2 - n->ctrl_mem.addr],
                               trans_len);
            }
        }
    }
    ...
    NvmeRequest *req)
{
    ...
    if (req->qsg.nsg > 0) {
        ...
        req->aiocb = req->is_write ?
            blk_aio_writev(n->conf.blk, aio_slba, &req->iov,
                           data_size >> 9, nvme_rw_cb, req) :
            blk_aio_readv(n->conf.blk, aio_slba, &req->iov,
                          data_size >> 9, nvme_rw_cb, req);
    }
    ...
    NvmeRequest *req)
{
    ...
    if (dw11 & NVME_DSMGMT_AD) {
        ...
        write_status = nvme_dma_write_prp(n, (uint8_t *)range, sizeof(range),
                                 prp1, prp2);

        if (write_status != NVME_SUCCESS) {
            nvme_set_error_page(n, req->sq->sqid, cmd->cid, NVME_INVALID_FIELD,
...
static uint16_t nvme_io_cmd(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    uint16_t ns_status = nvme_check_nsid(n, nsid);

    if (ns_status != NVME_SUCCESS) {
        return ns_status;
    }

    ns = n->ns_attached[nsid - 1];
    switch (cmd->opcode) {
    case NVME_CMD_WRITE:
    case NVME_CMD_READ:
...
static uint16_t nvme_get_feature(NvmeCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    case NVME_LBA_RANGE_TYPE:
        ns_status = nvme_check_nsid(n, nsid);
        if (ns_status != NVME_SUCCESS) {
            return ns_status;
        }
        rt = n->ns_attached[nsid - 1]->lba_range;
...
static uint16_t nvme_smart_info(NvmeCtrl *n, NvmeCmd *cmd, uint32_t buf_len)
{
    ...
    smart.data_units_written[0] =
        cpu_to_le64(stats->nr_bytes[BLOCK_ACCT_WRITE]);
...
static uint16_t nvme_format(NvmeCtrl *n, NvmeCmd *cmd)
{
    ...
    if (nsid == NVME_BROADCAST_NSID) {
        ...
        for (i = 0; i < ARRAY_SIZE(n->ns_attached); ++i) {
            ns = n->ns_attached[i];
            if (ns != NULL) {
...
static uint16_t nvme_set_db_memory(NvmeCtrl *n, const NvmeCmd *cmd)
{
    ...
    for (i = 1; i < n->num_queues; i++) {
        ...
        if (cq) {
            /* Completion queue head pointer location,
               (2 * QID + 1) * stride. */
